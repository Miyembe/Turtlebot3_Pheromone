...
class Node():
    ...
    def pheroCallback(self, message, cargs):
        ...
        if self.is_phero_inj is True:
            phero.gradInjection(x_idx, y_idx, 1, 0.6, 0.3, self.phero_max)
        ...
        distance_to_origin = sqrt((x-self.target_x)**2+(y-self.target_y)**2)
        if self.is_saved is False and distance_to_origin < 0.3:
            print("distance_to_origin: {}".format(distance_to_origin))
