...
class Node():
    ...
    def __init__(self):
        ...
        for i in range(self.num_robots):
            self.pheromone[i] = Pheromone('dynamic {}'.format(i), size = 12, res = 20, evaporation = 0.2, diffusion = 0)
        ...
        self.phero_name = 'tcds_exp4' # file name of the loaded pheromone
    ...
    def posToIndex(self, x, y):
        ...
        for i in range(self.num_robots):
            ...
            print("x_index[i]: {}, y_index[i]: {}".format(x_index[i], y_index[i]))
        ...
        if self.is_phero_inj is True:
            for i, phero_dy in enumerate(self.pheromone):
                if '%d'%i in phero_dy.name:
                    phero_dy.gradInjection(x_idx[i], y_idx[i], 0.5, 0.2, 1.0, self.phero_max)
            ...
            try:
                for i in range(len(self.pheromone)):  # Reset the pheromone grid
                    ...
                    if 'static' in self.pheromone[i].name:
                        self.pheromone[i].load(self.phero_name) # you can load any types of pheromone grid
                        print(self.phero_name + " is loaded.")
                print("Pheromone grid reset succesfully!")
