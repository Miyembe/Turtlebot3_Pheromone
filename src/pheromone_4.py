...
class Node():
    ...
    def pheroCallback(self, message, cargs):
        ...
        phero = cargs
        x = [pos[0].x, pos[1].x, pos[2].x, pos[3].x]
        y = [pos[0].y, pos[1].y, pos[2].y, pos[3].y]
        ...
        for n in range(self.num_robots):
            ...
            for i in range(3):
                for j in range(3):
                    phero_sum = 0.0
                    for p in range(len(self.pheromone)):
                        if p != n:
                            phero_sum += self.pheromone[p].getPhero(x_idx[n]+i-1, y_idx[n]+j-1)
                    phero_sum = min(1.0, phero_sum)
                    phero_val[n].append(phero_sum) # Read the other's pheromone
...
if __name__ == "__main__":
    ...
    Phero3 = Pheromone('3', 0.1, 0)
    Phero4 = Pheromone('4', 0.1, 0)
    Phero = [Phero1, Phero2, Phero3, Phero4]
    node1 = Node(Phero)
    rospy.spin()
