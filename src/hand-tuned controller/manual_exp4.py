...
class WaypointNavigation:
    ...
    def __init__(self):
        ...
        self.target = [[-3.0, 0.0], [3.0, 0.0], [-3.0, -2.5], [3.0, -2.5], [-3.0, 2.5], [3.0, -2.5]] # Two goal
        self.d_robots = 5.0
        self.target_index = 0
        ...
        self.sub_phero = rospy.Subscriber('/phero_value', fma, self.phero_ig)
    ...
    def Computation(self):
        ...
        for i in range(self.num_robots):            
            if (self.phero_sum[i] > self.pheroThr):
                pub_msg[i] = self.PheroOA(self.phero[i])
    ...
    def reset(self):
        ...
        if self.counter_step != 0:
            if (self.counter_collision != 0 and self.counter_success != 0 and self.counter_timeout != 0):
