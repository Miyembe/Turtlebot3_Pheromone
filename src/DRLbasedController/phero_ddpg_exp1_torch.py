...
class ActorCritic:
	def __init__(self, env, sess):
		...
		self.actor_model = Actor(self.env.obsevation_space.shape, self.env.action_space.shape, self.hid_list)
		self.target_actor_model = Actor(self.env.obsevation_space.shape, self.env.action_space.shape, self.hid_list)
		...
		self.critic_model = Critic(self.env.obsevation_space.shape, self.env.action_space.shape, 1, self.hid_list)
		self.target_critic_model = Critic(self.env.obsevation_space.shape, self.env.action_space.shape, 1, self.hid_list)
		self.critic_optim = optim.Adam(self.actor.parameters(), lr=self.learning_rate)
	...
	def _train_critic_actor(self, samples):
		...
		target_actions = self.target_actor_model(to_tensor(new_states), volatile=True)
		...
		Q_now = self.critic_model(to_tensor([cur_states, actions]), volatile=True)
		next_Q = self.target_critic_model(to_tensor([new_states, target_actions]), volatile=True)
		Q_target = to_tensor(rewards) + self.gamma*next_Q.reshape(future_rewards.shape[0]) * to_tensor(1 - dones)	

		td_errors = Q_target - Q_now.reshape(Q_now.shape[0])

		value_loss = nn.MSELoss(Q_target, Q_now)
		value_loss.backward()
	...
	def save_weight(self, num_trials, trial_len):
		torch.save(
            ...
            self.path + 'actormodel' + '-' +  str(num_trials) + '-' + str(trial_len) +'/actor.pkl'
        )
        torch.save(
            self.critic.state_dict(),
            self.path + 'actormodel' + '-' +  str(num_trials) + '-' + str(trial_len) + '/critic.pkl'.format(output)
        )
